...
@then('the new group list is equal to the old list with the added group', target_fixture='verify_group_added')
...
def non_empty_group_list(db, app):
    if len(db.get_group_list()) == 0:
        app.group.create(Group(name="testfortheemptyness"))
    return db.get_group_list()
...
@when('I delete a group from the list', target_fixture='delete_group')
